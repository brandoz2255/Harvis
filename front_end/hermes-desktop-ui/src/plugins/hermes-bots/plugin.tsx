...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // Harvis: SESSIONS | BOTS is always a tab strip. The Bots tab used to appear
    // only once a non-default profile existed, which hid Bot Mode (and its "+"
    // to make the first bot) from anyone who had not already found it. Bots is
    // the one bot surface now (the old /bots page's bots are adopted as profiles
    // server-side), so it is always reachable next to Sessions.
    ctx.register({
      id: 'pane',
      area: 'panes',
      title: 'Bots',
