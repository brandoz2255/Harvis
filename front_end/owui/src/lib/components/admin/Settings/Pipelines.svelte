...
>
	<Pane title={$i18n.t('Manage Pipelines')}>
		{#if PIPELINES_LIST !== null}
			{#if PIPELINES_LIST.length > 0}
				<Section>
					...
					<Row label={$i18n.t('Upload Pipeline')}>
					...
					<Row label={$i18n.t('Install from Github URL')}>
						<svelte:fragment slot="detail">
							<div class="flex w-full mt-2">
								<div class="flex-1 mr-2">
									<input
				...
				<Section title={$i18n.t('Pipelines Valves')}>
					{#if pipelines !== null}
						{#if pipelines.length > 0}
							<div class="py-3">
							...
							{#if pipelines[selectedPipelineIdx].valves}
								{#if valves}
									{#each Object.keys(valves_spec.properties) as property, idx}
										<Row label={valves_spec.properties[property].title}>
											<button
								...
								{/if}
							{:else}
								<div class="py-3">{$i18n.t('No valves')}</div>
							{/if}
						{:else if pipelines.length === 0}
							<div class="py-3">{$i18n.t('Pipelines Not Detected')}</div>
						{/if}
					{:else}
		...
		<svelte:fragment slot="actions">
			<!-- Svelte requires a slot fragment to be a direct child of the component,
			     so the condition lives inside it. Pane's actions bar collapses when
			     this renders nothing, so no empty border strip is left behind. -->
